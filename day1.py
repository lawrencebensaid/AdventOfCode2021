...
def sum_of_measurements(measurements, i):
    depth_past_1 = int(data[i - 1]) if i > 0 else 0
    depth_past_2 = int(data[i - 2]) if i > 1 else 0
    depth = int(data[i])
    return depth_past_1 + depth_past_2 + depth

for i in range(2, len(data) - 1):
    sum = sum_of_measurements(data, i)
    next_sum = sum_of_measurements(data, i + 1)
    if sum < next_sum:
        increase_count += 1
